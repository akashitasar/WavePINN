...
if __name__ == '__main__':
    ...
    u = u.reshape(t.shape)
    # --- TRUE SOLUTION ---
    u_true = np.cos(np.pi * x) * np.cos(np.pi * t)

    # --- ERROR ---
    error = np.abs(u - u_true)

    # --- L2 ERROR ---
    l2_error = np.sqrt(np.mean((u - u_true) ** 2))
    ...
    t_cross_sections = [0.25, 0.5, 0.75, 1]
